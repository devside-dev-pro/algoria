    ...
    <main style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, minHeight: '100vh' }}>
      <header className="panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, padding: '10px 14px' }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button onClick={() => sendCommand('kill')} style={{ ...pill(false), color: '#ff8aa2', borderColor: 'rgba(255,107,138,.45)' }}>
            kill
          </button>
        </div>
      </header>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 12 }}>
        <section className="panel" style={{ padding: 12, minHeight: 280, display: 'flex', flexDirection: 'column' }}>
          <div style={{ fontSize: 13, marginBottom: 6 }}>
            XAU/USD · M5 <span style={{ color: 'var(--dim)', fontSize: 11 }}>— chart Lightweight Charts (à brancher sur le flux Broadcast)</span>
          </div>
          <div style={{ flex: 1, display: 'grid', placeItems: 'center', color: 'var(--dim)', border: '1px dashed var(--border)', borderRadius: 8 }}>
            {st ? `atr ${fmt(st.atr)} · adx ${fmt(st.adx, 1)} · vol ${pct(st.atr_percentile)}` : 'en attente du runner…'}
          </div>
        </section>
        ...
        <section className="panel mono" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: '8px 12px', fontSize: 11.5, lineHeight: 1.8, height: 252, overflowY: 'auto' }}>
            {events.length === 0 && <span style={{ color: 'var(--dim)' }}>en attente du runner…</span>}
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 12 }}>
        {signals.length === 0 && <div className="panel" style={{ padding: 12, color: 'var(--dim)' }}>aucun signal pour l'instant</div>}
        {signals.map((s: any) => {
          ...
          return (
            <div key={s.id} className="panel" style={{ padding: 12, borderColor: long ? 'rgba(34,224,166,.3)' : 'rgba(255,107,138,.3)' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--muted)', display: 'flex', justifyContent: 'space-between' }}>
                <span>entrée {fmt(s.entry)}</span>
                <span>SL {fmt(s.stop_loss)}</span>
                <span>TP {fmt(s.take_profits?.[0])}</span>
              </div>
              <div style={{ fontSize: 11, color: 'var(--dim)', marginTop: 8 }}>
                conf {pct(s.confidence)} · R:R {fmt(s.risk_reward)} · {fmt(s.lot)} lot
              </div>
            </div>
          );
        })}
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 10 }}>
        <Metric label="Solde" value={st ? fmt(st.balance, 0) : '—'} />
        <Metric label="Equity" value={st ? fmt(st.equity, 0) : '—'} />
        <Metric label="P&L jour" value={st ? fmt(st.day_pnl, 0) : '—'} color={st && st.day_pnl >= 0 ? 'var(--up)' : 'var(--down)'} />
        <Metric label="Positions" value={st ? String(st.open_positions ?? 0) : '—'} />
        <Metric label="Expo risque" value={st ? pct(st.open_risk_pct) : '—'} />
