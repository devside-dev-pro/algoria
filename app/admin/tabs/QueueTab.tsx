...
export function QueueTab() {
  const { KIND_LABEL, actions, busy, connectViaSth, goLive, copyDepositInfo, creds, depInfoCopied, legalOf, moveViaSth, nameOf, openMember, pending, pendingTotal, post, recordDepositAfterConnect, rejectConnect, reveal, rows, setCreds, setTab, validateLots, waitBroker } = useAdmin();
  return (
              ...
              <div key={a.id} style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '10px 12px', borderRadius: 10, border: '1px solid var(--border)', background: 'rgba(10,17,31,.55)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  ...
                  {a.kind === 'connect' && !a.detail?.waiting_broker && (
                    <button disabled={busy} onClick={() => goLive(a)} title="one tap: validate the activation lot, connect the copier via STH, switch LIVE, log the deposit and the country — stops at the first failing step" style={{ ...okBtn, fontWeight: 800 }}>{pending === `golive:${a.id}` ? '⏳ CONNECTING…' : '⚡ GO LIVE'}</button>
                  )}
                  {a.kind === 'connect' && (
                    <button disabled={busy || !lotsCleared(a.detail as Record<string, unknown>)} onClick={() => connectViaSth(a)} title={lotsCleared(a.detail as Record<string, unknown>) ? 'connect this account to the copier via STH now, then go LIVE + log the deposit (one click, no manual STH entry)' : 'volume d\u2019activation pas encore validé — pointe le dashboard partenaire et clique ✓ LOTS'} style={{ ...okBtn, color: '#06121f', background: lotsCleared(a.detail as Record<string, unknown>) ? 'linear-gradient(90deg,#2be3f5,#2e8bf0)' : 'rgba(130,152,190,.2)', border: 'none', opacity: lotsCleared(a.detail as Record<string, unknown>) ? 1 : 0.5 }}>{pending === `sth:${a.id}` ? '⏳ CONNECTING…' : '🔗 CONNECT (STH)'}</button>
                  )}
                  {a.kind === 'strategy_change' && (
                    <button disabled={busy} onClick={() => moveViaSth(a.id)} title="move this member's receiver to their new strategy's master via the STH API (API-connected members only — manually-added receivers must be moved in the STH dashboard)" style={{ ...okBtn, color: '#06121f', background: 'linear-gradient(90deg,#2be3f5,#2e8bf0)', border: 'none' }}>{pending === `move:${a.id}` ? '⏳ MOVING…' : '🔀 MOVE (STH)'}</button>
                  )}
                  <button disabled={busy || (a.kind === 'connect' && !lotsCleared(a.detail as Record<string, unknown>))} onClick={() => post({ done: a.id }, () => { setCreds((c) => { const n = { ...c }; delete n[a.id]; return n; }); if (a.kind === 'connect') recordDepositAfterConnect(a); }, { key: `done:${a.id}`, drop: a.id })} title="mark CONNECTED without calling STH: member goes LIVE, gets a push, referral commission is created — use only if the copier was connected by hand (if you connected the account in STH yourself) — connect cards also offer to log the deposit" style={a.kind === 'connect' && !lotsCleared(a.detail as Record<string, unknown>) ? { ...okBtn, opacity: 0.4 } : okBtn}>{pending === `done:${a.id}` ? '⏳ …' : '✓ DONE'}</button>
                  {a.kind === 'connect' && (
                  ...
                  <button disabled={busy} onClick={() => post({ dismiss: a.id }, undefined, { key: `dismiss:${a.id}`, drop: a.id })} title="dismiss without applying anything (stale/spam card — no side effects)" style={miniBtn}>{pending === `dismiss:${a.id}` ? '⏳ …' : '✕ DISMISS'}</button>
                </div>
                {creds[a.id] && (
