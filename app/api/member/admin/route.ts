...
async function run(body: Body, s: AdminSession, req: NextRequest): Promise<NextResponse> {
  ...
  if (body.botBroadcast) {
    ...
    if (audience === 'pending') {
      ...
      targets = (ms ?? []) as typeof targets;
    } else if (audience === 'stalled') {
      // ── LE PLUS GROS TROU DU TUNNEL, ET IL ÉTAIT HORS DE PORTÉE (17/09/2026) ──────────────────────
      // Mesuré ce jour-là : 1 346 inscrits → 374 choisissent un broker → 129 déposent un dossier. Les
      // 245 qui manquent ont choisi leur broker puis ne sont jamais revenus avec leurs identifiants —
      // 65 % de ceux qui démarrent vraiment. C'est de loin le plus gros décrochage, et les deux seuls
      // segments existants (dossiers en file, membres live) ne peuvent pas les atteindre : ils ne sont
      // ni l'un ni l'autre. Ils étaient donc injoignables en un geste, alors qu'ils sont le premier
      // gisement du produit.
      const { data: ms } = await db.from('members').select('tg_id,member_no,tg_name,locale').eq('status', 'onboarding').not('broker', 'is', null).is('banned_at', null);
      const { data: submitted } = await db.from('member_actions').select('tg_id').eq('kind', 'connect');
      const hasSubmitted = new Set((submitted ?? []).map((r) => Number(r.tg_id)));
      targets = ((ms ?? []) as typeof targets).filter((m) => !hasSubmitted.has(Number(m.tg_id)));
    } else return NextResponse.json({ error: 'unknown audience' }, { status: 400 });
    ...
    for (const m of targets) {
      ...
      try {
        ...
        // 'stalled' n'a JAMAIS vu l'accueil : le renvoyer sur /member l'y ferait rebondir vers l'onboarding.
        // On l'envoie directement là où il s'est arrêté — un clic de moins sur le seul segment qui décroche.
        const bMarkup = body.botBroadcast.cta ? ctaKeyboard(asLocale(m.locale), audience === 'stalled' ? '/member/onboarding' : '/member') : undefined;
        const r = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          method: 'POST', headers: { 'content-type': 'application/json' },
