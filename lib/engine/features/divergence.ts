...
/** RSI divergence: price and momentum disagree → reversal. */
export const divergence: Feature = {
  ...
  compute({ bars }: FeatureInput): FeatureScore | null {
    ...
    if (lows.length === 2 && lows[1].price < lows[0].price && r[lows[1].index] > r[lows[0].index])
      return { key: 'divergence', score: 1, strength: clamp01((r[lows[1].index] - r[lows[0].index]) / 15 + 0.4), note: 'bullish divergence' };
    if (highs.length === 2 && highs[1].price > highs[0].price && r[highs[1].index] < r[highs[0].index])
      return { key: 'divergence', score: -1, strength: clamp01((r[highs[0].index] - r[highs[1].index]) / 15 + 0.4), note: 'bearish divergence' };
