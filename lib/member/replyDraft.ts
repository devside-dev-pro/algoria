...
function facts(): string {
  ...
  return [
    ...
    `THE RULE THAT GETS PEOPLE REFUSED (say it plainly whenever a broker comes up): an account opened directly on the broker's website, or at a broker that is not a partner, is REFUSED at connection — the broker does not count it as Algoria's, and Algoria is paid by the broker. Only two ways in: open a new account through the partner link in the app, or, for an existing account at a partner broker, ask that broker's support to attach it to Algoria's affiliate ID (the app shows the exact message to send). Accounts at other brokers cannot be attached.`,
