...
//
// ── S2 REMISE À $500 LE 01/09, ANNULÉE LE 02/09 ─────────────────────────────────────────────────────
// Passée à $500 puis remise à $200 le lendemain, en production entre-temps. La leçon vaut d'être écrite,
// parce que rien dans le code ne l'empêchait de se reproduire.
//
// LA CONDITION EST, ET RESTE : « on remet $500 QUAND S1 EST DE RETOUR ». Pas avant. S1 est toujours en
// maintenance, donc S1 ne tient toujours pas le palier $200, donc passer S2 à $500 ferme purement et
// simplement l'entrée à $200 — c'est la panne du 20/08 refaite à l'envers, et cette fois volontairement.
// Le chiffre de risque plus bas (un compte à $200 encaisse −23,4 % sur la pire journée du maître, contre
// −9,4 % à $500) est exact et il ne change rien : il justifie de REMONTER le palier le jour où une
// stratégie à $200 existe pour accueillir ces gens, pas de leur claquer la porte au nez en attendant.
//
// ET SURTOUT — CE QUE LE CHIFFRE NE DIT PAS : des membres déposent $200 aujourd'hui, et pour une partie
// d'entre eux (Afrique du Sud notamment) $200 n'est pas un premier pas prudent, c'est LE MAXIMUM qu'ils
// peuvent mettre. On ne leur demande pas de faire machine arrière du jour au lendemain. Un minimum qui
// monte pendant que des gens sont en train de déposer n'est pas un arbitrage de risque, c'est une porte
// fermée sur des gens déjà entrés.
//
// DONC : ne pas remonter S2 à $500 tant que `inMaintenance(1)` est vrai. Le jour où S1 rouvre, le palier
// $200 existe de nouveau et le changement devient ce qu'il aurait toujours dû être — un déplacement de
